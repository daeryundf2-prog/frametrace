...
fn inventory_commands_emit_bounded_sqlite_backed_json() {
    ...
    #[cfg(unix)]
    {
        let outside_symlink_target = root.join("dangling-symlink-target.json");
        let symlink_manifest_path = case_dir.join("reports/dangling-inventory-export.json");
        let _ = fs::remove_file(&outside_symlink_target);
        let _ = fs::remove_file(&symlink_manifest_path);
        std::os::unix::fs::symlink(&outside_symlink_target, &symlink_manifest_path)
            .expect("dangling symlink should be created");

        let symlink_export = run(&[
            "inventory-export-manifest",
            path(&case_dir),
            "--operator",
            "qa",
            "--output",
            path(&symlink_manifest_path),
            "vid_000001",
        ]);
        assert_failure_contains(&symlink_export, "cannot be a symlink");
        assert!(!outside_symlink_target.exists());
    }
