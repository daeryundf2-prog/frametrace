// Verify stage-3 embedded viewer + summary strip. Prereqs: app on :8477,
// headless Edge on :9222. Run: node scripts/verify-stage3-embed.mjs
const CDP = 'http://127.0.0.1:9222';
const BASE = 'http://127.0.0.1:8477';
const CASE = 'D:/frametrace-e01-test/case-synth';
const SHOTS = process.argv[2] || 'shots-user';
const fs = await import('fs');
let nextId = 1;
const pending = new Map();
function connect(url) { return new Promise((res, rej) => { const w = new WebSocket(url); w.onopen = () => res(w); w.onerror = rej; }); }
function send(ws, method, params = {}, sessionId) {
  const id = nextId++;
  return new Promise((resolve, reject) => { pending.set(id, { resolve, reject }); ws.send(JSON.stringify({ id, method, params, sessionId })); });
}
function wire(ws) { ws.onmessage = e => { const m = JSON.parse(e.data); if (m.id && pending.has(m.id)) { const { resolve, reject } = pending.get(m.id); pending.delete(m.id); m.error ? reject(new Error(m.error.message)) : resolve(m.result); } }; }
async function ev(ws, sid, expr) {
  const r = await send(ws, 'Runtime.evaluate', { expression: expr, returnByValue: true, awaitPromise: true, userGesture: true }, sid);
  if (r.exceptionDetails) throw new Error(JSON.stringify(r.exceptionDetails.exception?.description || r.exceptionDetails.text));
  return r.result.value;
}
const sleep = ms => new Promise(r => setTimeout(r, ms));
const version = await (await fetch(CDP + '/json/version')).json();
const ws = await connect(version.webSocketDebuggerUrl); wire(ws);
const { targetId } = await send(ws, 'Target.createTarget', { url: BASE + '/' });
const { sessionId: sid } = await send(ws, 'Target.attachToTarget', { targetId, flatten: true });
await send(ws, 'Page.enable', {}, sid);
await sleep(2500);
fs.mkdirSync(SHOTS, { recursive: true });
async function shot(name) { const r = await send(ws, 'Page.captureScreenshot', { format: 'png' }, sid); fs.writeFileSync(`${SHOTS}/${name}.png`, Buffer.from(r.data, 'base64')); console.log('  [shot]', name); }

// open case → auto-jump to stage 3
await ev(ws, sid, `(() => { const el = document.getElementById('caseDir'); el.value = '${CASE}'; el.dispatchEvent(new Event('input')); document.getElementById('btnOpenCase').click(); return 1; })()`);
await sleep(4000);

const info = await ev(ws, sid, `(() => {
  const f = document.getElementById('reviewFrame');
  return { src: f && f.src, stage3: document.getElementById('resultCard').classList.contains('on'),
    frameH: f ? f.getBoundingClientRect().height : 0,
    stats: document.getElementById('caseStats').textContent.trim() };
})()`);
console.log('stage3 info:', JSON.stringify(info));

// wait for iframe + summary getter
let stats = '';
for (let i = 0; i < 25; i++) {
  await sleep(800);
  const r = await ev(ws, sid, `(() => {
    const f = document.getElementById('reviewFrame');
    const fn = f && f.contentWindow && f.contentWindow.__frametraceSummary;
    if (typeof fn !== 'function') return null;
    return { summary: fn(), strip: document.getElementById('caseStats').textContent.trim() };
  })()`);
  if (r) { stats = r; break; }
}
console.log('summary from iframe:', JSON.stringify(stats));
await sleep(3000); // let strip render
await shot('40-stage3-embed');

// mark a record inside the iframe, verify strip updates
await ev(ws, sid, `(() => { const f = document.getElementById('reviewFrame');
  const w = f.contentWindow; w.document.dispatchEvent(new w.KeyboardEvent('keydown', { key: '1', bubbles: true })); return 1; })()`);
await sleep(3500);
const stats2 = await ev(ws, sid, `document.getElementById('caseStats').textContent.trim()`);
console.log('strip after iframe mark:', stats2);
await shot('41-stage3-embed-marked');
ws.close();
process.exit(0);
