...
- **Mutating requests require the per-instance nonce even without a configured token.** The server generates a 128-bit instance nonce at startup (written to the user-private instance file and reported as `instance` by `/api/status`), plants it as a `ft_nonce` cookie (`HttpOnly; SameSite=Strict`) on GET responses, and requires it on every non-GET/HEAD request via the `X-FrameTrace-Nonce` header, the cookie, or a `?nonce=` query value. Same-origin page fetches carry the cookie automatically; cross-site form POSTs cannot (SameSite=Strict strips it) and non-browser clients that never learned the nonce are refused with `401`. Scripts read the nonce from the instance file or `/api/status`. **A local process that can read the user's instance file or loopback responses can still invoke any endpoint** — the nonce raises the bar for web-content attackers (CSRF, malicious pages, rebound origins), not for local malware, which already holds examiner-equivalent privileges; `FRAMETRACE_TOKEN` remains the stronger gate for shared machines.
