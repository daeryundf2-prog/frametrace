...
| GUI-INV-004 | Avoid rendering all rows into DOM. | Prototype now uses fixed-height virtualized rows over the filtered result set. | `gui/evidence-viewer/app.js`, `gui/evidence-viewer/styles.css` | DOM rows <= viewport rows + overscan for 10k fixture. | DOM count measurement during scroll. |
| GUI-INV-005 | Prove prototype scale before production shell work. | Prototype now generates 10,000 deterministic mock records. | Deterministic mock generator in prototype. | 10,000 deterministic rows with stable IDs and mixed states. | Browser performance log and mock count check. |
...
| GUI-INV-009 | Require auditable bulk preview before mutation. | Prototype bulk actions now render a non-mutating preview with preview ID, operator, filter snapshot, expected mutation, warning, and audit path. | Bulk action preview UI and engine command preview. | Every bulk action displays selected count, filters, expected mutation, operator action, warnings, and audit output path before mutation. | Bulk preview QA checklist. |
...
| GUI-INV-013 | Use one SQLite-backed production inventory API contract. | Rust CLI now exposes bounded inventory list/search/facet/detail, non-mutating bulk preview, and inventory export manifest. Prototype still uses in-memory records only for UI interaction proof. | Rust engine, `case.db`, API/query functions, WinUI shell adapter. | `list_inventory`, `search_inventory`, `inventory_facets`, `get_file_detail`, `bulk_preview`, `apply_bulk_action`, and `export_manifest` all page results and return query/audit metadata. | API contract tests and SQLite fixture query-plan evidence. |
...
- [x] 1440 px default row count measured.
- [x] 1920 px default row count measured.
- [x] Inventory-focused row count measured.
- [x] 10k mock fixture exists.
- [x] DOM row bound measured.
- [x] 100k search latency measured.
- [x] 1M SQLite search latency measured.
- [x] Bulk preview audited.
...
Captured on 2026-06-17 with Chrome headless against `gui/evidence-viewer/index.html`.

| Scenario | Result | Evidence |
| --- | --- | --- |
| 10k mock inventory | `recordsLength = 10000` | `output/playwright/frametrace-inventory-1440-current.png` |
| 1440 px default density | 13 estimated visible rows; 30 DOM rows including overscan | `output/playwright/frametrace-inventory-1440-current.png` |
| 1920 px default density | 18 estimated visible rows; 35 DOM rows including overscan | `output/playwright/frametrace-inventory-1920-current.png` |
| 1440 px inventory-focused mode | 30 estimated visible rows; 47 DOM rows including overscan | `output/playwright/frametrace-inventory-focus-1440-current.png` |
| Bulk preview drawer | Preview ID, operator, selected rows, filters, expected mutation, warning, and audit path visible | `output/playwright/frametrace-inventory-bulk-preview-current.png` |
| Production large-case policy | Prototype displays SQLite paged-query policy and does not present the 10k mock array as production transport | `output/playwright/frametrace-inventory-1440-current.png` |
| 100k SQLite performance QA | passed; max query 4 ms; full scan count 0; max RSS 9,240,576 bytes | `.omo/ulw-loop/frame-gui-20260617102845/evidence/performance-report-100k.json` |
| 1M SQLite performance QA | passed; max query 570 ms; full scan count 0; max RSS 9,338,880 bytes | `.omo/ulw-loop/frame-gui-20260617102845/evidence/performance-report-1m.json` |
