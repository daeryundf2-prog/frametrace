...
use crate::util::{now_unix, read_to_string, unique_path};
...
pub fn export_video(
    ...
    let tool = run_ffmpeg_export(&source_path, &output_path, options)?;
    log::write_export_log(
        case_dir,
        selector,
        &source_path,
        &output_path,
...
fn run_ffmpeg_export(
    ...
    let tool = resolve_external_tool(&options.ffmpeg_bin, &["ffmpeg"], "-version")?;
