...
fn detect_named_vendor(path: &str, _extension: &str) -> Option<SourceProfile> {
    let checks: &[(&[&str], &str, &str, &str, &str, &str)] = &[
        (
            ...
            "hikvision",
            "medium",
            "Path signal only: prefer exported media first and verify with file metadata or vendor player before making a vendor claim.",
        ),
        (
            &["dahua", "smartpss", "dss"],
            "cctv-nvr-export",
            ...
            "dahua_dav",
            "medium",
            "Path signal only: use DAV/ASF detection and official player-assisted conversion when FFmpeg cannot decode.",
        ),
        (
            &["blackvue"],
            "dashcam-sd-card",
            ...
            "blackvue",
            "medium",
            "Path signal only: pair front/rear channels and extract GPS/G-sensor metadata when present.",
        ),
        (
            ...
            "thinkware_inavi",
            "medium",
            "Path signal only: classify recording mode folders and extract GPS/speed metadata when present.",
        ),
        (
            &["finevu", "fine-vu"],
            "dashcam-sd-card",
            ...
            "finevu",
            "medium",
            "Path signal only: extract player-compatible GPS/G-sensor metadata when present.",
        ),
        (
            &["iroad"],
            "dashcam-sd-card",
            ...
            "iroad",
            "medium",
            "Path signal only: classify event folders and GPS metadata when present.",
        ),
        (
            &["viofo"],
            "dashcam-sd-card",
            ...
            "viofo",
            "medium",
            "Path signal only: use MP4 as primary media and extract GPS tracks when present.",
        ),
        (
            &["garmin"],
            "dashcam-sd-card",
            ...
            "garmin_dashcam",
            "medium",
            "Path signal only: preserve DCIM layout, MP4 files, and GLV companion files.",
        ),
        (
            &["nextbase"],
            "dashcam-sd-card",
            ...
            "nextbase",
            "medium",
            "Path signal only: extract GPS map/G-sensor metadata when present.",
        ),
        (
            &["hanwha", "wisenet", "/wave/"],
            "cctv-nvr-export",
            ...
            "hanwha_wisenet_wave",
            "medium",
            "Path signal only: prefer WAVE official export path; ingest standard MP4/MKV/AVI outputs directly.",
        ),
        (
            &["axis", "vapix"],
            "cctv-nvr-export",
            ...
            "axis_onvif",
            "medium",
            "Path signal only: use MKV exports directly; device/API acquisition is a future connector path.",
        ),
        (
            &["onvif"],
            "cctv-nvr-export",
        ...
        (
            ...
            "uniview",
            "medium",
            "Path signal only: start with official export outputs; add proprietary samples later.",
        ),
        (
            &["idis"],
            "cctv-nvr-export",
            ...
            "idis",
            "medium",
            "Path signal only: ingest AVI directly; preserve self-player exports without running them.",
        ),
        (
            &["bosch", "bvms", "vrm"],
            "cctv-nvr-export",
            ...
            "bosch_bvms",
            "medium",
            "Path signal only: detect ZIP/native exports and ingest ASF/MOV/MP4 derived media directly.",
        ),
        (
            &["milestone", "xprotect"],
            "cctv-nvr-export",
            ...
            "milestone_xprotect",
            "medium",
            "Path signal only: detect XProtect packages; ingest AVI/MKV exports directly.",
        ),
        (
            &["genetec"],
            "cctv-nvr-export",
            ...
            "genetec_g64",
            "medium",
            "Path signal only: ingest MP4/ASF directly; preserve G64/G64x for player-assisted review.",
        ),
        (
            &["avigilon", "/acc/"],
            "cctv-nvr-export",
            ...
            "avigilon_ave",
            "medium",
            "Path signal only: ingest AVI directly; preserve AVE exports for player-assisted review.",
        ),
        (
            &["milesight"],
            "cctv-nvr-export",
            ...
            "milesight",
            "medium",
            "Path signal only: ingest standard MP4/AVI/MKV/ASF exports directly; preserve EXE packages.",
        ),
        (
            &["honeywell"],
            "dvr-hdd-recovery",
